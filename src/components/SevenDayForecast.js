...
const SevenDayForecast = () => {
    ...
    const [selectedCity, setSelectedCity] = useState('Amsterdam'); // Default city
    ...
    const getUniqueDates = () => {
        ...
        return Object.keys(uniqueDates).slice(0, 7); // Selecteer de eerste 7 unieke datums
    };
            ...
            {weatherData && (
                <table>
                    ...
                    {getUniqueDates().map((date) => {
                        ...
                        const dayWeather = weatherData.list.find((item) => item.dt_txt.includes(date)).weather[0].description;
                        return (
                            <tr key={date}>
